...
* Single source shortest path - shortest path from source node to all other nodes
& Doesnot work with negative weights
* Works with both directed and undirected graph.
& Time Complexity : 
! Array : O(V²)
! Fibonacci heap as priority queue : O(E+Vlog₂V)
! Binary heap as priority queue : O(Elog₂V)
*/
